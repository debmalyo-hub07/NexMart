...
const productSchema = z.object({
  ...
  taxRateBps: z.number().int().min(0).max(10000).nullable().optional().transform(value => value ?? undefined),
  hsnCode: z.string().trim().regex(/^\d{4,8}$/).or(z.literal('')).optional().transform(value => value || undefined),
  returnWindowDays: z.number().int().min(0).max(90).nullable().optional().transform(value => value ?? undefined),
  ...
  }).refine(variant => variant.comparePrice === undefined || variant.comparePrice >= variant.price, { message: 'MRP cannot be less than the selling price', path: ['comparePrice'] })).min(1).max(100).refine(variants => new Set(variants.map(variant => variant.sku)).size === variants.length, 'Each variant needs a unique SKU'),
