...
export default function CartPage() {
  ...
  return <main id="main-content" className="store-page"><div className="page-container">
    ...
    <div className="grid gap-7 lg:grid-cols-[minmax(0,1fr)_340px]">
      ...
      {items.length > 0 && <aside className="card h-fit space-y-5 lg:sticky lg:top-24" aria-label="Order summary"><h2 className="text-xl">Order summary</h2><OrderTotals totals={totals} /><ShoppingPolicyLinks /><p className="text-xs text-muted">Shipping is free on a product subtotal above ₹999. GST is calculated at 18%.</p>{blocked ? <p className="text-sm text-amber-200" role="status">Resolve the cart updates above before continuing.</p> : <Link href="/checkout" className="btn-primary w-full">Continue to checkout</Link>}<Link href="/products" className="btn-secondary w-full">Continue shopping</Link></aside>}
    </div>
