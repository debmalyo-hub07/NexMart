...
import { ShoppingPolicyLinks } from '@/components/layout/ShoppingPolicyLinks';
...
export default function CheckoutPage() {
  ...
  const totals = calculateTotals(cart.items.map(item => ({ price: getCartItemState(item).price, quantity: item.quantity, taxRateBps: item.product?.taxRateBps })));
  ...
  return <main id="main-content" className="store-page"><div className="page-container">
    ...
    {error && <div role="alert" className="mb-6 rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-800"><p>{error}</p><Link href="/orders" className="mt-2 inline-flex min-h-11 items-center underline">Check your orders</Link></div>}
    {recovering || !cart.ready && !cart.error ? <p role="status" className="flex items-center gap-2 py-10 text-secondary"><Loader2 className="animate-spin" size={20} aria-hidden />Checking checkout information…</p>
      ...
      : <FormProvider {...form}><form onSubmit={form.handleSubmit(submit)} noValidate><fieldset disabled={busy || recovering} className="grid min-w-0 gap-7 lg:grid-cols-[minmax(0,1fr)_340px]">
        ...
        </section><fieldset className="card"><legend className="px-1 text-xl font-semibold">Payment method</legend><div className="grid gap-3 sm:grid-cols-2">{[{ value: 'online', title: 'Pay online', text: 'Continue to Razorpay after the order is saved.' }, { value: 'cod', title: 'Cash on delivery', text: 'Pay the order total when your order arrives.' }].map(option => <label key={option.value} className={`flex cursor-pointer items-start gap-3 rounded-xl border p-4 ${method === option.value ? 'border-[var(--accent-violet)] bg-[rgba(18,62,117,0.06)]' : 'border-[var(--border-control)]'}`}><input type="radio" name="paymentMethod" checked={method === option.value} onChange={() => setMethod(option.value as 'online' | 'cod')} className="mt-1 h-5 w-5 shrink-0 accent-[#123E75]" /><span><span className="block text-sm font-medium">{option.title}</span><span className="mt-1 block text-xs text-secondary">{option.text}</span></span></label>)}</div></fieldset></div>
        <aside className="card h-fit space-y-5 lg:sticky lg:top-24"><h2 className="text-xl">Review your order</h2><ul className="space-y-3">{cart.items.map(item => <li key={item._id} className="flex justify-between gap-3 text-sm"><div className="min-w-0"><p className="break-words">{item.product?.name}</p><p className="break-words text-xs text-muted">{item.variant} · Quantity {item.quantity}</p>{item.seller?.storefrontName && <p className="text-xs text-[var(--accent-violet)]">Sold by {item.seller.storefrontName}</p>}{getCartItemState(item).priceChanged && <p className="text-xs text-amber-700">Updated price shown</p>}</div><span className="shrink-0 font-mono">{formatPrice(getCartItemState(item).price * item.quantity)}</span></li>)}</ul><OrderTotals totals={totals} /><ShoppingPolicyLinks /><p className="text-xs text-secondary">{method === 'online' ? 'Your order will be saved first. You can then complete payment on its order page.' : 'Your order will be placed and will await store confirmation.'}</p><button type="submit" disabled={blocked || busy} className="btn-primary w-full">{busy ? <Loader2 size={18} className="animate-spin" aria-hidden /> : <CreditCard size={18} aria-hidden />}{busy ? 'Saving order…' : uncertain ? 'Retry this checkout' : method === 'online' ? 'Place order & continue' : 'Place order'}</button><Link href="/cart" className="inline-flex min-h-11 items-center text-sm text-secondary underline">Return to cart</Link></aside>
