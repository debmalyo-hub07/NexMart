...
export default function OrdersPage() {
  ...
  return <main id="main-content" className="store-page"><div className="page-container">
    <PageHeader title="Your orders" description="Track deliveries, check payments, and download order receipts." actions={<button type="button" className="btn-secondary" onClick={() => void query.refetch()} disabled={query.isFetching}><RefreshCw size={17} aria-hidden />Refresh</button>} />
    ...
    {query.isPending ? <div className="space-y-3">{Array.from({ length: 3 }, (_, index) => <OrderRowSkeleton key={index} />)}</div> : query.data?.data?.length ? <div className="space-y-5">{query.data.data.map(order => <article className="card" key={order._id}>
      ...
      <div className="mt-5 flex flex-wrap items-start gap-3 border-t border-[var(--border)] pt-4"><Link href={`/orders/${order._id}`} className="btn-primary">{order.paymentMethod === 'online' && order.orderStatus === 'placed' && ['pending', 'failed'].includes(order.paymentStatus) ? 'Payment & order details' : 'View order & tracking'}</Link>{(order.invoiceUrl || order.paymentStatus === 'paid' || order.orderStatus === 'delivered') && <InvoiceButton orderId={order._id} invoiceUrl={order.invoiceUrl} />}</div>
