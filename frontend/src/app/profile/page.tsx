...
export default function ProfilePage() {
  ...
  async function signOutEverywhere() {
    ...
    try {
      ...
      // This session is dead too, so leave through the normal logout path
      // rather than stranding the store on a session the server rejects.
  ...
  return <main id="main-content" className="store-page"><div className="page-container"><PageHeader title="Your account" description="Manage your profile, delivery addresses, and sign-in details." actions={<><Link href="/orders" className="btn-secondary">Your orders</Link><Link href="/wishlist" className="btn-secondary">Saved products</Link></>} />
    {error && <p role="alert" className="mb-5 rounded-xl border border-red-300 bg-red-50 p-4 text-sm text-red-700">{error}</p>}
    {query.isError ? <QueryError label="Your profile" onRetry={() => void query.refetch()} /> : query.isPending ? <ProfileSkeleton /> : profile && <Tabs.Root defaultValue="profile">
      <Tabs.List aria-label="Account sections" className="mb-6 flex gap-1 overflow-x-auto rounded-xl border border-[var(--border)] p-1">{['profile', 'addresses', 'security'].map(tab => <Tabs.Trigger key={tab} value={tab} className="min-h-11 flex-1 rounded-lg px-4 text-sm capitalize text-secondary data-[state=active]:bg-[var(--accent-violet)] data-[state=active]:text-white">{tab}</Tabs.Trigger>)}</Tabs.List>
      <Tabs.Content value="profile" className="grid gap-6 md:grid-cols-[240px_minmax(0,1fr)]"><section className="card h-fit"><div className="mb-4 flex items-center gap-3">{profile.profilePicture ? <Image src={profile.profilePicture} alt="" width={56} height={56} className="rounded-full object-cover" /> : <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-[rgba(18,62,117,0.10)] font-semibold text-[var(--accent-violet)]">{getInitials(profile.name)}</span>}<p className="min-w-0 break-words font-medium">{profile.name}</p></div><label htmlFor="profile-photo" className="field-label">Profile photo</label><input id="profile-photo" type="file" accept="image/jpeg,image/png,image/webp" className="block w-full min-w-0 text-sm text-secondary file:mr-2 file:min-h-11 file:rounded-lg file:border-0 file:bg-[#EEECE5] file:px-3 file:text-[var(--text-primary)]" disabled={avatarBusy} onChange={event => { void upload(event.target.files?.[0]); event.currentTarget.value = ''; }} /><p className="field-hint">JPEG, PNG or WebP. Up to 5 MB.</p>{avatarBusy && <p role="status" className="text-sm text-secondary">Uploading photo…</p>}</section>
      ...
      <Tabs.Content value="security" className="card max-w-2xl"><h2 className="mb-5 text-xl">Sign-in and security</h2>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          {profile.authProviders?.includes('email')
            ...
            : <button type="button" className="btn-secondary" onClick={() => setAddPasswordOpen(true)}>Add a password</button>}
    ...
    <SetPasswordModal isOpen={addPasswordOpen} onClose={() => setAddPasswordOpen(false)} onDone={() => void query.refetch()} />
