...
export default function AdminUsersPage() {
  ...
  const columns: Column<Record<string, unknown>>[] = [
    ...
    { key: 'phone', header: 'Phone', render: (r) => <span className="text-secondary text-sm">{(r.phone as string) || '—'}</span> },
    {
    ...
    { key: 'createdAt', header: 'Joined', render: (r) => <span className="text-muted text-xs">{formatDate(r.createdAt as string)}</span>, sortable: true },
  ];

  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div>
        <h1 className="font-outfit text-2xl font-bold text-white">Customers</h1>
          <p className="text-muted text-sm mt-1">{isError ? 'Customer data unavailable' : `${data?.meta?.total ?? 0} registered customers`}</p>
      </div>
        ...
        actions={(row) => (
          <button
            ...
            className={(row.isActive
              ? 'flex min-h-11 min-w-11 items-center justify-center rounded-lg text-muted hover:text-red-400 hover:bg-red-500/10 transition-colors disabled:opacity-50'
              : 'flex min-h-11 min-w-11 items-center justify-center rounded-lg text-muted hover:text-acid-400 hover:bg-acid-400/10 transition-colors disabled:opacity-50')}
        ...
        expandableRender={(row) => {
          ...
          return (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              <div>
                <p className="text-muted uppercase text-xs tracking-wider mb-1">Default Address</p>
                {address ? (
                  <div className="text-white/80">
                    ...
                    <p className="text-muted text-xs mt-1">Phone: {address.phone}</p>
                  </div>
                ) : (
                  <p className="text-muted italic">No addresses saved</p>
                )}
              </div>
              <div>
                <p className="text-muted uppercase text-xs tracking-wider mb-1">Account Info</p>
