...
export function OrderItems({ items }: { items: OrderItem[] }) {
  return <ul className="divide-y divide-[var(--border)]">{items.map((item, index) => <li key={`${item.variant}-${index}`} className="flex items-start gap-3 py-4 first:pt-0 last:pb-0">
    ...
    <div className="min-w-0 flex-1"><p className="break-words text-sm font-medium">{item.product?.slug ? <Link href={`/products/${item.product.slug}`} className="hover:text-[var(--accent-violet-light)]">{item.name || item.product.name}</Link> : item.name || item.product?.name || 'Product no longer listed'}</p><p className="mt-1 break-words text-xs text-muted">{item.variant} · Quantity {item.quantity}</p><p className="mt-1 text-xs text-secondary">{formatPrice(item.unitPrice)} each</p><p className="mt-2 font-mono text-sm sm:hidden">{formatPrice(item.totalPrice)}</p></div>
