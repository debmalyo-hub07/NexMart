...
const menuItem = 'flex min-h-11 items-center gap-3 rounded-lg px-3 text-sm text-secondary outline-none data-[highlighted]:bg-[rgba(18,62,117,.09)] data-[highlighted]:text-[var(--text-primary)]';
...
export function Navbar() {
  ...
  return <>
    ...
    <header className="fixed inset-x-0 top-0 z-50 bg-white shadow-[0_1px_3px_rgba(15,23,42,.08)]">
      ...
      <nav className="page-container flex h-16 items-center gap-2 md:h-[72px] md:gap-6" aria-label="Main navigation">
        ...
        <Link href={admin ? '/admin' : '/'} aria-label="NexMart home" className="mr-auto flex min-h-11 shrink-0 items-center gap-2 md:mr-3"><Logo size={30} /><span className="font-outfit text-2xl font-semibold tracking-tight text-[#123058]">NexMart<span className="text-orange-500">.</span></span></Link>
        ...
        <div className="flex shrink-0 items-center gap-1 md:gap-3">
          ...
          {mounted && authenticated ? <Dropdown.Root><Dropdown.Trigger asChild><button type="button" className="icon-button gap-2 px-2" aria-label="Account menu"><User size={21} aria-hidden /><span className="hidden text-sm xl:inline">Account</span><ChevronDown size={12} className="hidden xl:block" aria-hidden /></button></Dropdown.Trigger><Dropdown.Portal><Dropdown.Content className="z-[80] w-64 rounded-xl border border-[var(--border)] bg-white p-2 shadow-[0_16px_44px_rgba(15,23,42,.18)]" align="end" sideOffset={8} collisionPadding={16}>
            ...
            <Dropdown.Separator className="my-1 h-px bg-[var(--border)]" /><Dropdown.Item asChild><button type="button" className={`${menuItem} w-full text-red-600`} onClick={() => void logout()}><LogOut size={17} aria-hidden />Sign out</button></Dropdown.Item>
          ...
          {!admin && <button type="button" className="icon-button relative gap-2 px-2" aria-label={`Open cart${mounted && count ? `, ${count} items` : ''}`} onClick={() => setCartOpen(true)}><ShoppingBag size={21} aria-hidden /><span className="hidden text-sm xl:inline">Bag</span>{mounted && count > 0 && <span className="absolute -right-1 -top-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-orange-500 px-1 text-[10px] font-semibold text-white">{count > 99 ? '99+' : count}</span>}</button>}
        </div>
      </nav>
      ...
      <div className="category-bar hidden lg:block"><nav className="page-container flex h-11 items-center gap-5" aria-label="Shop departments">
        ...
        <Link href="/products" className={cn('border-l border-[var(--border)] pl-5 text-sm', pathname === '/products' ? 'text-orange-600 font-medium' : 'text-secondary')}>All products</Link>
        <div className="flex min-w-0 flex-1 items-center gap-6 overflow-x-auto no-scrollbar">{roots.slice(0, 6).map(category => <Link key={category._id} href={`/categories/${category.slug}`} aria-current={pathname === `/categories/${category.slug}` ? 'page' : undefined} className="text-xs text-secondary transition-colors hover:text-[var(--text-primary)]">{category.name}</Link>)}</div>
        <Link href="/products?featured=true" className="shrink-0 text-xs font-medium text-orange-600">The curated edit ↗</Link>
      </nav></div>
    </header>
    ...
    <Overlay open={navOpen} onClose={() => setNavOpen(false)} title="Explore NexMart" variant="drawer-left">
      <nav aria-label="Mobile navigation" className="space-y-5">
        ...
        <div className="border-t border-[var(--border)] pt-4"><p className="eyebrow mb-2 px-4">Shop departments</p>{categories.isError ? <QueryError label="Categories" onRetry={() => void categories.refetch()} /> : categories.isPending ? <p role="status" className="px-4 text-sm text-muted">Loading categories…</p> : roots.map(category => <details key={category._id} className="border-b border-[var(--border)]"><summary className="flex min-h-12 cursor-pointer items-center gap-3 text-sm"><CategoryIcon name={category.name} size={19} />{category.name}<ChevronDown size={15} className="ml-auto" aria-hidden /></summary><Link href={`/categories/${category.slug}`} className="nav-item" onClick={() => setNavOpen(false)}>Explore all {category.name}</Link>{(categories.data ?? []).filter(child => parentId(child) === category._id).map(child => <Link key={child._id} href={`/categories/${child.slug}`} className="nav-item text-xs" onClick={() => setNavOpen(false)}>{child.name}</Link>)}</details>)}</div>
        <div className="border-t border-[var(--border)] pt-4"><p className="eyebrow mb-2 px-4">Your account</p>{mounted && authenticated ? <>{accountLinks.map(({ href, label, Icon }) => <Link key={href} href={href} onClick={() => setNavOpen(false)} className="nav-item"><Icon size={18} aria-hidden />{label}</Link>)}<button type="button" className="nav-item w-full text-red-600" onClick={() => { setNavOpen(false); void logout(); }}><LogOut size={18} aria-hidden />Sign out</button></> : <Link href="/customer/login" className="btn-primary w-full" onClick={() => setNavOpen(false)}>Sign in to your account</Link>}</div>
      </nav>
    </Overlay>
    {!admin && <CartDrawer />}
  </>;
