...
export function Footer() {
  return <footer className="mt-8 border-t border-white/15 bg-space-800">
    <div className="page-container grid gap-7 border-b border-white/15 py-7 md:grid-cols-3">{[{ Icon: Search, title: 'A clearer choice', copy: 'Useful specifications, options, and side-by-side comparison.' }, { Icon: CreditCard, title: 'Review before you pay', copy: 'Item prices, shipping, and tax shown before ordering.' }, { Icon: ShoppingBag, title: 'Keep it all together', copy: 'Saved finds, order details, and delivery updates.' }].map(({ Icon, title, copy }) => <div key={title} className="flex items-start gap-3"><Icon size={23} className="mt-1 shrink-0 text-violet-200" aria-hidden /><div><p className="text-sm font-medium">{title}</p><p className="mt-1 text-xs leading-relaxed text-muted">{copy}</p></div></div>)}</div>
    <div className="page-container grid gap-8 py-10 sm:grid-cols-2 lg:grid-cols-[1.4fr_1fr_1fr_1.2fr] lg:py-12">
      ...
      {Object.entries(groups).map(([title, items]) => <nav key={title} aria-label={title}><h2 className="mb-3 text-sm font-semibold">{title}</h2><ul>{items.map(item => <li key={item.href}><Link href={item.href} className="inline-flex min-h-11 items-center text-sm text-muted hover:text-white">{item.label}</Link></li>)}</ul></nav>)}
    </div>
    <div className="border-t border-white/15"><div className="page-container flex flex-wrap items-center justify-between gap-3 py-5 text-xs text-muted"><p>© {new Date().getFullYear()} NexMart</p><p>INR · Online payments via Razorpay · COD where available</p></div></div>
  </footer>;
}
