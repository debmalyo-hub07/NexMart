...
export function HomeDiscovery() {
  return <div className="mt-7 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-secondary"><span className="text-muted">Find your direction</span><Link className="inline-flex min-h-11 items-center gap-1 hover:text-white" href="/categories/electronics">Everyday tech <ArrowUpRight size={13} aria-hidden /></Link><Link className="inline-flex min-h-11 items-center gap-1 hover:text-white" href="/categories/home">A home refresh <ArrowUpRight size={13} aria-hidden /></Link></div>;
}
...
export function HomeSpotlight({ products }: { products?: Product[] }) {
  ...
  return <article className="hero-product" data-paused={paused || undefined} onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocus={() => setPaused(true)} onBlur={event => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setPaused(false); }}>
    <div className="flex items-center justify-between gap-3 px-5 py-4"><p className="eyebrow text-violet-200">In the curated edit</p><span className="font-mono text-xs text-muted">0{index + 1} / 0{choices.length}</span></div>
...
export function HomeMoreProducts({ products, arrivals }: { products?: Product[]; arrivals?: ApiResponse<Product[]> }) {
  ...
  return <section id="the-edit" className="border-y border-white/10 bg-space-950/30"><div className="page-container section">
...
export function HomeEditorial({ categories }: { categories?: Category[] }) {
  ...
  return <section className="page-container section" aria-label="Shop the collections"><div className="grid gap-5 md:grid-cols-2">{edits.map(edit => {
    ...
    return <article key={edit.slug} className="editorial-card"><div className="relative z-10 flex max-w-[65%] flex-col items-start p-6 sm:p-8"><p className="eyebrow text-violet-200">{edit.eyebrow}</p><h2 className="mt-4 whitespace-pre-line text-2xl leading-tight sm:text-3xl">{edit.title}</h2><Link href={`/categories/${edit.slug}`} className="mt-auto inline-flex min-h-11 items-center gap-2 pt-5 text-sm text-secondary">{edit.link}<ArrowUpRight size={17} aria-hidden /></Link></div><div className="absolute bottom-0 right-0 top-0 w-[43%]">{category?.image ? <ProductImage src={category.image} alt="" sizes="300px" className="p-3" /> : <div className="flex h-full items-center justify-center text-violet-200"><Grid2X2 size={72} strokeWidth={1} aria-hidden /></div>}</div></article>;
  })}</div></section>;
}
