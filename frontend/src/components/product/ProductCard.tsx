...
export const ProductCard = memo(function ProductCard({ product, className }: { product: Product; className?: string }) {
  ...
  return <article className={cn('product-tile group', className)}>
    <div className="relative">
      ...
      {product.isDemo ? <span className="product-label">Sample</span> : discount > 0 ? <span className="product-label">{discount}% off</span> : product.isFeatured && <span className="product-label">Curated pick</span>}
      {!admin && <button type="button" disabled={wishlistLoading} aria-label={`${saved ? 'Remove' : 'Save'} ${name}${saved ? ' from saved products' : ''}`} aria-pressed={saved} onClick={() => toggleWishlist(product._id)} className="product-save"><Heart size={18} className={saved ? 'fill-current text-violet-700' : ''} aria-hidden /></button>}
    ...
    <div className="flex flex-1 flex-col p-3 sm:p-5">
      ...
      <h3 className="min-h-[2.8rem] text-sm leading-snug sm:text-base"><Link href={href} className="line-clamp-2 hover:text-violet-200">{name}</Link></h3>
      ...
      {product.ratings?.count > 0 && <div className="mt-2 flex items-center gap-1.5 text-xs"><Star size={13} className="fill-amber-400 text-amber-400" aria-hidden /><span>{product.ratings.average.toFixed(1)} <span className="text-muted">({product.ratings.count} reviews)</span></span></div>}
      ...
      <p className="mb-4 mt-1.5 flex items-center gap-1 text-xs text-secondary">{product.isDemo ? 'Preview only · not for sale' : inStock ? <><Check size={13} className="text-acid-400" aria-hidden />{hasOptions ? `${product.variants.length} options` : 'Available to order'}</> : hasOptions ? 'Selected option is out of stock' : 'Currently out of stock'}</p>
      ...
      <label className="mt-3 flex min-h-11 cursor-pointer items-center justify-center gap-2 border-t border-white/10 pt-2 text-xs text-secondary"><input type="checkbox" checked={compared} onChange={() => { if (!toggleCompare(product)) toast('Compare up to 3 products. Remove one to add another.', 'info'); }} aria-label={`Compare ${name}`} className="h-4 w-4 accent-violet-500" />{compared ? 'Added to compare' : 'Compare'}</label>
    </div>
