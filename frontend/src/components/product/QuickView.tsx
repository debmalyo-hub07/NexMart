  ...
  return <Overlay open onClose={onClose} returnFocus={returnFocus} title="A closer look" description={productName(product)} className="max-w-4xl">
    <div className="grid gap-6 sm:grid-cols-2">
      ...
      <div className="min-w-0 space-y-5">
        <div><p className="eyebrow">{product.brand || product.category?.name}</p><h2 className="mt-2 text-2xl">{productName(product)}</h2><p className="mt-4 font-mono text-2xl">{variant ? formatPrice(variant.price) : 'Unavailable'}</p>{product.isDemo && <p className="mt-2 text-xs text-violet-200">Sample product · illustrative price · not for sale</p>}</div>
