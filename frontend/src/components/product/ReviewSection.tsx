...
export function ReviewSection({ productId, ratings }: { productId: string; ratings: { average: number; count: number } }) {
  ...
  return <section className="border-t border-[var(--border)] pt-8">
    ...
    {open && <form onSubmit={form.handleSubmit(values => { if (pending.current) return; pending.current = true; submit.mutate(values); })} className="card mb-6 space-y-4" noValidate><fieldset disabled={submit.isPending} className="space-y-4"><legend className="mb-4 text-lg font-semibold">Your review</legend>
      <fieldset><legend className="field-label">Rating</legend><div className="flex flex-wrap gap-2">{[1, 2, 3, 4, 5].map(value => <label key={value} className="flex min-h-11 cursor-pointer items-center gap-2 rounded-lg border border-[var(--border-control)] px-3 text-sm"><input type="radio" value={value} {...form.register('rating')} className="h-4 w-4 accent-violet-500" />{value}<Star size={14} aria-hidden /><span className="sr-only">{value === 1 ? 'star' : 'stars'}</span></label>)}</div></fieldset>
