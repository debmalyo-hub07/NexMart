...
export function CatalogFilters({ params, categories, facets, categorySlug, instant = false, apply }: { params: URLSearchParams; categories: Category[]; facets?: CatalogFacets; categorySlug?: string; instant?: boolean; apply: (values: Record<string, string>) => void }) {
  ...
  return <form onSubmit={event => { event.preventDefault(); submit(); }}>
    ...
    <details open className="filter-group"><summary>Price range<ChevronDown size={15} aria-hidden /></summary>
      ...
      <div className="mt-3 flex flex-wrap gap-2">{[999, 4999, 19999].map(price => <button key={price} type="button" className="min-h-11 rounded-lg border border-[var(--border-control)] px-2.5 text-xs text-secondary hover:border-[var(--accent-violet)] hover:text-[var(--accent-violet)]" onClick={() => { const values = { ...draft, minPrice: '', maxPrice: String(price) }; setDraft(values); if (instant) submit(values); }}>Under {formatPrice(price)}</button>)}</div>
    </details>
