    ...
    {query.isPending ? <div className="grid gap-8 lg:grid-cols-2"><Skeleton className="aspect-square rounded-2xl" /><div className="space-y-5"><Skeleton className="h-24" /><Skeleton className="h-16" /><Skeleton className="h-44" /></div></div> : query.isError ? <QueryError label="Product details" onRetry={() => void query.refetch()} /> : !product ? <EmptyState title="This product is unavailable" description="Explore the other finds in our collection." action={<Link className="btn-primary" href="/products">Browse products</Link>} /> : <>
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,1fr)] lg:gap-12">
        ...
        <div className="min-w-0">
          <div className="mb-4 flex flex-wrap items-center gap-2">{product.brand && <Link href={`/products?brand=${encodeURIComponent(product.brand)}`} className="eyebrow inline-flex min-h-11 items-center text-orange-600">{product.brand}</Link>}{product.isDemo ? <span className="badge-neutral">Sample collection</span> : product.isFeatured && <span className="badge-violet">In the curated edit</span>}</div>
          ...
          <div className="my-6 border-y border-[var(--border)] py-5"><div className="flex flex-wrap items-baseline gap-3">{variant ? <p className="font-mono text-3xl font-semibold">{formatPrice(variant.price)}</p> : <p className="text-secondary">Price unavailable</p>}{discount > 0 && !product.isDemo && <><del className="font-mono text-sm text-muted"><span className="sr-only">Previous price </span>{formatPrice(variant!.comparePrice!)}</del><span className="badge-acid">{discount}% off</span></>}</div><p className="mt-2 text-xs text-muted">{product.isDemo ? 'Illustrative INR price · This sample cannot be purchased.' : 'Price includes GST (18%). The delivery charge, if any, is shown in your cart.'}</p></div>
          <VariantSelector variants={product.variants ?? []} selectedSku={variant?.sku ?? ''} onSelect={choose} sample={product.isDemo} />
          {product.isDemo ? <div className="mt-5 rounded-xl border border-[var(--border)] bg-[#FAFAF8] p-4"><p className="flex items-center gap-2 text-sm font-medium"><Info size={17} aria-hidden />A little inspiration, not live inventory.</p><p className="mt-2 text-xs leading-relaxed text-secondary">This product helps you explore the catalog and compare designs. Availability, seller terms, warranties, and prices must be confirmed before it can be sold.</p><Link href="/products?inStock=true" className="mt-2 inline-flex min-h-11 items-center gap-2 text-sm text-orange-600">Browse products available to buy <ArrowRight size={15} aria-hidden /></Link></div> : <p className={`mt-4 flex items-center gap-2 text-sm ${variant?.stock ? 'text-emerald-700' : 'text-secondary'}`} role="status">{variant?.stock ? <Check size={17} aria-hidden /> : <Info size={17} aria-hidden />}{variant?.stock ? variant.stock < 5 ? `${variant.stock} available for this option` : 'In stock for this option' : 'This option is currently unavailable'}</p>}
