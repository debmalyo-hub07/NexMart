    ...
    {query.isPending ? <div className="grid gap-8 lg:grid-cols-2"><Skeleton className="aspect-square rounded-2xl" /><div className="space-y-5"><Skeleton className="h-24" /><Skeleton className="h-16" /><Skeleton className="h-44" /></div></div> : query.isError ? <QueryError label="Product details" onRetry={() => void query.refetch()} /> : !product ? <EmptyState title="This product is unavailable" description="Explore the other finds in our collection." action={<Link className="btn-primary" href="/products">Browse products</Link>} /> : <>
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,1fr)] lg:gap-12">
        ...
        <div className="min-w-0">
          <div className="mb-4 flex flex-wrap items-center gap-2">{product.brand && <Link href={`/products?brand=${encodeURIComponent(product.brand)}`} className="eyebrow inline-flex min-h-11 items-center text-[var(--accent-violet)]">{product.brand}</Link>}{product.isDemo ? <span className="badge-neutral">Sample collection</span> : product.isFeatured && <span className="badge-violet">In the curated edit</span>}</div>
          <h1 className="text-3xl font-medium leading-tight sm:text-4xl">{name}</h1>
          {!product.isDemo && <a href="#product-reviews" className="mt-3 inline-flex min-h-11 items-center gap-2 text-sm"><span className="inline-flex items-center gap-1 rounded bg-emerald-600 px-2 py-0.5 text-xs font-medium text-white"><Star size={11} className="fill-current" aria-hidden />{product.ratings?.count ? product.ratings.average.toFixed(1) : '—'}</span><span className="text-secondary">{product.ratings?.count ? `${product.ratings.count} ${product.ratings.count === 1 ? 'review' : 'reviews'}` : 'No reviews yet'}</span></a>}
          <p className="mt-4 text-sm leading-relaxed text-secondary">{product.description}</p>
          {highlights.length > 0 && <dl className="mt-6 grid grid-cols-2 gap-x-5 gap-y-4 rounded-xl border border-[var(--border)] bg-[#FAFAF8] p-4">{highlights.map(([key, value]) => <div key={key}><dt className="text-xs text-muted">{key}</dt><dd className="mt-1 break-words text-sm font-medium">{value}</dd></div>)}</dl>}
          <div className="my-6 border-y border-[var(--border)] py-5"><div className="flex flex-wrap items-baseline gap-3">{variant ? <p className="font-mono text-3xl font-semibold">{formatPrice(variant.price)}</p> : <p className="text-secondary">Price unavailable</p>}{discount > 0 && !product.isDemo && <><del className="font-mono text-sm text-muted"><span className="sr-only">Previous price </span>{formatPrice(variant!.comparePrice!)}</del><span className="badge-acid">{discount}% off</span></>}</div><p className="mt-2 text-xs text-muted">{product.isDemo ? 'Illustrative INR price · This sample cannot be purchased.' : 'Price includes GST. The delivery charge, if any, is shown in your cart.'}</p></div>
          <VariantSelector variants={product.variants ?? []} selectedSku={variant?.sku ?? ''} onSelect={choose} sample={product.isDemo} />
          {product.isDemo ? <div className="mt-5 rounded-xl border border-[var(--border)] bg-[#FAFAF8] p-4"><p className="flex items-center gap-2 text-sm font-medium"><Info size={17} aria-hidden />A little inspiration, not live inventory.</p><p className="mt-2 text-xs leading-relaxed text-secondary">This product helps you explore the catalog and compare designs. Availability, seller terms, warranties, and prices must be confirmed before it can be sold.</p><Link href="/products?inStock=true" className="mt-2 inline-flex min-h-11 items-center gap-2 text-sm text-[var(--accent-violet)]">Browse products available to buy <ArrowRight size={15} aria-hidden /></Link></div> : <p className={`mt-4 flex items-center gap-2 text-sm ${variant?.stock ? 'text-emerald-700' : 'text-secondary'}`} role="status">{variant?.stock ? <Check size={17} aria-hidden /> : <Info size={17} aria-hidden />}{variant?.stock ? variant.stock < 5 ? `${variant.stock} available for this option` : 'In stock for this option' : 'This option is currently unavailable'}</p>}
          <div className="mt-5 space-y-3">{admin ? <Link href={`/admin/products/${product._id}/edit`} className="btn-secondary w-full">Edit this product</Link> : <div className="flex gap-3">{!!variant?.stock && !product.isDemo && <div className="flex items-center rounded-lg border border-[var(--border-control)]" role="group" aria-label="Quantity"><button type="button" className="icon-button" aria-label="Decrease quantity" disabled={qty <= 1} onClick={() => setQuantity(qty - 1)}><Minus size={16} aria-hidden /></button><span className="min-w-6 text-center font-mono text-sm" aria-live="polite">{qty}</span><button type="button" className="icon-button" aria-label="Increase quantity" disabled={qty >= Math.min(10, variant.stock)} onClick={() => setQuantity(qty + 1)}><Plus size={16} aria-hidden /></button></div>}<AddToCartButton product={product} variant={variant} quantity={qty} className="min-h-12 flex-1" /></div>}
      ...
      <section id="related-products" className="policy-content mt-10 border-t border-[var(--border)] pt-8"><div className="mb-6 flex items-end justify-between gap-3"><div><p className="eyebrow mb-2">Keep exploring</p><h2 className="section-heading">A few more possibilities.</h2></div>{product.category && <Link href={`/categories/${product.category.slug}`} className="inline-flex min-h-11 items-center gap-2 text-sm text-secondary"><span className="hidden sm:inline">Explore {product.category.name}</span><ArrowRight size={19} aria-hidden /><span className="sr-only sm:hidden">Explore this category</span></Link>}</div>{related.isError ? <QueryError label="Related products" onRetry={() => void related.refetch()} /> : related.isPending ? <div className="product-grid">{Array.from({ length: 4 }, (_, i) => <ProductCardSkeleton key={i} />)}</div> : !related.data?.some(item => item._id !== product._id) ? <p className="py-6 text-sm text-muted">More products will appear here as this collection grows.</p> : <div className="product-grid">{related.data.filter(item => item._id !== product._id).slice(0, 4).map(item => <ProductCard key={item._id} product={item} />)}</div>}</section>
    </>}
  </div></main>;
}
