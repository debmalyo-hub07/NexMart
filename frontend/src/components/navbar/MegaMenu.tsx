...
export function MegaMenu({ categories }: { categories: Category[] }) {
  ...
  return <Popover.Root open={open} onOpenChange={setOpen}>
    ...
    <Popover.Portal><Popover.Content className="z-[80] w-[min(860px,calc(100vw-48px))] overflow-hidden rounded-2xl border border-white/25 bg-space-800" sideOffset={9} align="start" collisionPadding={24}>
      {roots.length ? <Tabs.Root defaultValue={roots[0].slug} orientation="vertical" className="grid grid-cols-[240px_minmax(0,1fr)]">
        <Tabs.List aria-label="Departments" className="border-r border-white/15 bg-space-900 p-3">{roots.map(category => <Tabs.Trigger key={category._id} value={category.slug} className="flex min-h-12 w-full items-center gap-3 rounded-lg px-3 text-left text-sm text-secondary data-[state=active]:bg-violet-500/15 data-[state=active]:text-white"><CategoryIcon name={category.name} size={18} />{category.name}</Tabs.Trigger>)}</Tabs.List>
        {roots.map(category => <Tabs.Content key={category._id} value={category.slug} className="p-7">
          <div className="flex gap-5"><div className="flex-1"><p className="eyebrow">Find your next favourite</p><h2 className="mt-2 text-2xl">{category.name}</h2><p className="mt-3 text-sm leading-relaxed text-muted">{category.description}</p></div>{category.image && <div className="product-stage relative h-28 w-28 shrink-0 overflow-hidden rounded-xl"><ProductImage src={category.image} alt="" sizes="112px" className="p-2" /></div>}</div>
          <div className="mt-5 grid grid-cols-2 gap-x-4">{categories.filter(child => parentId(child) === category._id).map(child => <Link key={child._id} href={`/categories/${child.slug}`} onClick={() => setOpen(false)} className="flex min-h-11 items-center justify-between gap-3 border-b border-white/10 text-sm text-secondary hover:text-white">{child.name}<span className="text-xs text-muted">{child.productCount ?? ''}</span></Link>)}</div>
