  ...
  return <Popover.Root open={open} onOpenChange={setOpen}>
    ...
    <Popover.Portal><Popover.Content className="z-[80] w-[min(860px,calc(100vw-48px))] overflow-hidden rounded-2xl border border-[var(--border)] bg-white shadow-[0_18px_50px_rgba(15,23,42,.18)]" sideOffset={9} align="start" collisionPadding={24}>
      {roots.length ? <Tabs.Root defaultValue={roots[0].slug} orientation="vertical" className="grid grid-cols-[240px_minmax(0,1fr)]">
        <Tabs.List aria-label="Departments" className="border-r border-[var(--border)] bg-[#FAFAF8] p-3">{roots.map(category => <Tabs.Trigger key={category._id} value={category.slug} className="flex min-h-12 w-full items-center gap-3 rounded-lg px-3 text-left text-sm text-secondary data-[state=active]:bg-[rgba(18,62,117,.10)] data-[state=active]:font-medium data-[state=active]:text-[var(--accent-violet)]"><CategoryIcon name={category.name} size={18} />{category.name}</Tabs.Trigger>)}</Tabs.List>
