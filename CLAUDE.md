...
### P4 — polish & consistency
Analytics missing skeleton (isLoading destructured, unused) · RevenueChart/analytics bar no empty state · `alert()` invoice → toast with download progress · `confirm()` category delete → ConfirmDialog · DataTable search debounce + server-side sort + mobile card view · toast timer/queue rewrite · `/products` pagination prev/next · product pagination pages 11+ unreachable · hero stat row wraps at 360px · `transition-all` purge · skeleton shimmer repaint · admin nested-scroll iOS fight · mega-menu fixed 580px at lg · dead deps purge (radix unused→now used, react-dropzone, react-image-crop, date-fns, cva, Sentry-or-config, Playwright CI or tests) · `error.tsx`'s nonexistent `btn-glow` · `about` page's dead `/auth/register` link · unused exports/imports cleanup · `loading.tsx` per admin sub-segment matching actual page shape · `not-found.tsx` with search + top categories.
