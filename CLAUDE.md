...
> ⚠️ **Upstash status (2026-09-05):** the `UPSTASH_REDIS_REST_URL` in `.env` (`lucky-gobbler-82201.upstash.io`) no longer resolves in public DNS — the instance appears deleted/renamed (Upstash free tier auto-deletes inactive databases). Since the resilience fix the API **degrades gracefully** with Redis down: rate limiting, JWT blacklisting, failed-login lockouts, and caches fail open with loud logging, a 2.5s call bound, no retries, and a **circuit breaker** (after 3 consecutive failures all Upstash calls reject instantly for 30s, then one probe — recovery is automatic when the URL is fixed). All endpoints serve real MongoDB data. Update the URL to a live Upstash database to restore full protection.
