...
- Query defaults (`providers.tsx`): retries via `src/lib/queryPolicy.ts` (never 4xx; 429 once respecting Retry-After; 5xx/network twice with exponential backoff), `refetchOnWindowFocus` + `refetchOnReconnect` on. The shared error branch for every query consumer is `<QueryError>` (`src/components/common/QueryError.tsx`) — DataTable takes `isError`/`onRetry`/`errorMessage`.
...
- Response envelope: `{success, message, data}` + `meta:{page,limit,total,totalPages}`. Failures: `{success:false, message, code, requestId?, errors?}` — machine-readable `code` on every error path, `requestId` for correlation (matches the `X-Request-Id` response header), and Zod field errors under `errors`. `getApiError` (`src/lib/api.ts`) surfaces the first field error ahead of the generic message. Health: `/health/live` (liveness), `/health/ready` (readiness — 503 until Mongo is connected), `/health` (compat alias).
...
> ✅ **Upstash RESTORED (2026-09-05):** the deleted free-tier database was replaced (`dynamic-werewolf-100212.upstash.io`) and verified live — rate limiting counts down (99→98 per request), JWT blacklisting, login lockouts, OTP storage, and caches are all active. The circuit breaker from the resilience fix remains as protection: any future Redis outage degrades gracefully (fail-open, 2.5s bound, no retries) and recovers automatically. A weekly keep-alive GitHub Action (`.github/workflows/redis-keepalive.yml`, secrets `UPSTASH_REDIS_REST_URL`/`_TOKEN`) pings the database so the free tier never goes idle again.
...
**Known-open from the audit (accepted, revisit before any non-React consumer):** reviews store HTML unsanitized (inert in the React app — zero `dangerouslySetInnerHTML`); ~~registration/OTP-verify responses permit email enumeration~~ **fixed 2026-09-11** (uniform opaque responses; see CHANGELOG); ~~`/products?category=<slug>` 400s~~ **fixed 2026-09-11** (shared `resolveCategoryFilter` accepts slugs on `/products` + `/search`); agent registration still 400s "Agent already exists" (weak oracle, low value behind pending-approval); some product-form Zod messages are raw defaults; ~~`backend npm run lint` references an uninstalled eslint~~ **fixed 2026-09-09** (flat-config ESLint 10 in CI); the Razorpay **dashboard webhook was created in normal/live mode while the keys are test-mode** — test payments never fire it (the reaper reconciles; see `docs/DEPLOYMENT.md` for creating it in the right mode).
...
### P7 — responsive/motion/reliability wave + review — ✅ DONE 2026-09-11 (plan: `docs/superpowers/plans/2026-09-10-responsive-motion-stabilization.md`; full list in `docs/CHANGELOG.md` 2026-09-11)
