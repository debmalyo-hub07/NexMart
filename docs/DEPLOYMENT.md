...
> ⚠️ **Create the webhook in the SAME mode as your API keys.** Razorpay keeps Test-mode and Live-mode webhooks separate: a webhook created while the dashboard is in normal/live mode never receives events from test-mode payments (and vice versa). With the current test keys (`rzp_test_*`), create it under **Test mode**; when you switch to live keys, add the Live-mode webhook too, with the same secret. The backend verifies one secret, so both modes can point at the same endpoint. Without a matching-mode webhook, payment confirmation relies solely on the client-side verify call — the order reaper reconciles pending online orders against Razorpay every 15 minutes, so paid-but-unconfirmed orders self-heal, but the webhook is the real-time source of truth.
>
> ⚠️ **Webhooks that fail for 24 hours are auto-disabled** (any non-2xx response or >5s timeout counts as a failure; deliveries retry with exponential backoff for 24h first; re-enabling is manual from the dashboard, and Razorpay emails the alert address you configured). This is why a webhook pointed at `localhost` *always* ends up disabled — Razorpay's servers can never reach your machine, every delivery fails, and the 24h window closes with a 100% failure rate. Verified live during the 2026-09-07 audit: the account's only test-mode webhook (an old ngrok URL, wrong path) was auto-disabled on 2026-05-20 after ~3 days.
>
> **Local development (why your test webhooks keep dying):** do NOT create a dashboard webhook pointing at `localhost:4000` — raw localhost/`:4000` URLs are rejected, and even a tunnel URL dies as soon as the tunnel is down, restarting the failure clock. For local dev, skip the webhook entirely: the client-side verify call confirms test payments fine, and the reaper (fixed 2026-09-07 — it previously crashed on every sweep) backstops within 15 minutes. If you want a true webhook locally, use `ngrok http 4000` and point the webhook at `https://<tunnel-id>.ngrok-free.app/api/v1/webhooks/razorpay` **for the session only**, then delete the webhook when done. Create the durable webhook once the backend has its public Render URL — that endpoint stays up, deliveries succeed, and the auto-disable never triggers.
